const secondParser = d3.timeParse("%H:%M:%S"),
  hourParser = d3.timeParse("%H:%M"),
  hourFormatter = d3.timeFormat("%H:%M"),
  bisectDate = d3.bisector((d) => d.time).left,
  margin = { top: 15, right: 35, bottom: 15, left: 35 };

const errorPromise = d3.csv("data/errors.csv").then((errors) =>
  errors.map((e) => {
    return {
      start: secondParser(e.start),
      end: secondParser(e.end),
...
function updateChart(selectedGroup) {
  const dataPromise = d3.csv(`data/${selectedGroup}.csv`).then((rows) => {
    const newRows = rows.map((r) => {
      return { ...r, time: hourParser(r.time) };
    });
    newRows.columns = rows.columns;
    return newRows;
  });

  Promise.all([errorPromise, dataPromise]).then((results) => {
    const errorTimes = results[0].filter((r) => r.errorCode == selectedGroup);
    ...
    svg
      ...
      .call(d3.axisBottom(x).tickFormat(hourFormatter));
    ...
    // set red error bands along the x-axis
    ...
    var buildVersions = keys.map(function (id) {
      return {
        id: id,
        values: data.map((d) => {
          return { time: d.time, errors: +d[id] };
        }),
      };
    });
    ...
    var buildVersion = svg.selectAll(".buildVersions").data(buildVersions);
